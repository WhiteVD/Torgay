...
namespace QuickApp.Server.Controllers
{
    public class AuthorizationController : Controller
    {
        ...
        private readonly OidcUserManager _oidcUserManager;
            ...
            IOptions<TokenValidatorOptions> tokenValidatorOptions)
        {
            ...
            _oidcUserManager = oidcUserManager;
        ...
        public async Task<IActionResult> Exchange()
        {
            ...
            else if (request.GrantType == OidcServerManager.ExtensionGrantType)
            {
                ...
                var validatorType = _tokenValidatorOptions.GetValidator(provider);
                ...
                if (!tokenValidationResult.IsValid)
                {
                    return GetForbidResult(
                        $"The user token failed validation. {tokenValidationResult.ErrorDescription}");
                }

                var providerSubject = tokenValidationResult.Subject!;
                var email = tokenValidationResult.Email!;
                var claims = tokenValidationResult.Claims!;

                var userResult = await _oidcUserManager
                    .FindOrCreateAsync(provider, providerSubject, email, claims, password);

                if (userResult.User == null)
                    return GetForbidResult(userResult.ErrorMessage ?? string.Empty,
                        Errors.InvalidGrant, userResult.ErrorData);
        ...
        private ForbidResult GetForbidResult(string errorDescription, string error = Errors.InvalidGrant,
            Dictionary<string, string>? errorData = null)
        {
