...
namespace Torgay.Server.Controllers
{
    ...
    {
        private readonly string _publicRoleName = appSettings.Value.DefaultUserRole;
        ...
        public async Task<IActionResult> GetUserHasPasswordById(string id)
        {
            ...
            if (appUser != null)
            {
                var userHasPassword = await userAccountService.GetUserHasPasswordAsync(appUser);
        ...
        {
            var appUser = _mapper.Map<ApplicationUser>(user);
            var result = await userAccountService
        ...
        public async Task<IActionResult> SendConfirmEmail()
        {
            ...
            if (!result.success)
                return StatusCode(StatusCodes.Status500InternalServerError,
                    "A server error occurred whilst sending confirmation email");
        ...
        public async Task<IActionResult> ConfirmEmail(string userId, string code)
        {
            ...
            var result = await userAccountService.ConfirmEmailAsync(appUser, code);
            if (!result.Succeeded)
            {
        ...
        public async Task<IActionResult> RecoverPassword([FromBody] UserPasswordRecoveryVM recoveryInfo)
        {
            ...
            if (recoveryInfo.UsernameOrEmail.Contains('@'))
                appUser = await userAccountService.GetUserByEmailAsync(recoveryInfo.UsernameOrEmail);

            appUser ??= await userAccountService.GetUserByUserNameAsync(recoveryInfo.UsernameOrEmail);

            if (appUser == null || !await userAccountService.GetIsEmailConfirmedAsync(appUser))
                return Accepted(); // Don't reveal that the user does not exist or is not confirmed

            var code = await userAccountService.GeneratePasswordResetTokenAsync(appUser);
            var baseUrl = Utilities.GetBaseUrl(Request);

            await emailSender.SendPasswordResetEmailAsync(baseUrl, code, appUser.UserName!, appUser.Email!);
        ...
        public async Task<IActionResult> ResetPassword([FromBody] UserPasswordResetVM resetInfo)
        {
            ...
            if (resetInfo.UsernameOrEmail.Contains('@'))
                appUser = await userAccountService.GetUserByEmailAsync(resetInfo.UsernameOrEmail);

            appUser ??= await userAccountService.GetUserByUserNameAsync(resetInfo.UsernameOrEmail);

            if (appUser == null)
                return NoContent(); // Don't reveal that the user does not exist

            var result = await userAccountService.ResetPasswordAsync(appUser, resetInfo.ResetCode, resetInfo.Password);
            if (!result.Succeeded)
            {
        ...
        {
            var code = await userAccountService.GenerateEmailConfirmationTokenAsync(appUser);
            var baseUrl = Utilities.GetBaseUrl(Request);

            return await emailSender.SendEmailConfirmationEmailAsync(baseUrl, code,
