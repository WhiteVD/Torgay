...
namespace Torgay.Core.Services.Account
{
    public class UserAccountService : IUserAccountService
    {
        ...
        public async Task<bool> GetUserHasPasswordAsync(ApplicationUser user)
        {
        ...
        {
            // "+" in token was incorrectly UrlDecoded along the way into a space. So replace back with a plus
            if (token.Contains(' '))
                token = token.Replace(" ", "+");
        ...
        public async Task<bool> GetIsEmailConfirmedAsync(ApplicationUser user)
        {
