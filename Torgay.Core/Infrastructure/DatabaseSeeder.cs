        ...
        private async Task SeedDemoDataAsync()
        {
            if (!await dbContext.Customers.AnyAsync() //&& !await dbContext.ProductCategories.AnyAsync()
                ...
                var freeLicense = new License {
                    Name = "Бесплатная лицензия",
                    Description = "Базовая лицензия с ограничением на 1 пользователя и 1 организацию",
                    Type = LicenseType.Free,
                    MaxUsers = 1,
                    MaxOrganizations = 1,
                    Price = 0,
                    DurationDays = 36500, // 100 лет
                    IsActive = true
                };

                var paidLicense = new License {
                    Name = "Платная лицензия",
                    Description = "Расширенная лицензия с неограниченным количеством пользователей и организаций",
                ...
                dbContext.Licenses.Add(freeLicense);
                dbContext.Licenses.Add(paidLicense);
