...
import { Component, OnInit, OnDestroy, inject, input, output, viewChild } from '@angular/core';
...
@Component({
    ...
    imports: [FormsModule, ReactiveFormsModule, MatFormField, MatLabel, MatInput, MatError, MatCheckbox]
})
      ...
      .subscribe({
        next: user => {
          setTimeout(() => {
            ...
            if (!this.isModal()) {
              this.alertService.showMessage(this.gT('app.Login'), this.gT('app.WelcomeUser', { userName: user.userName }),
                MessageSeverity.success);
            } else {
              this.alertService.showMessage(this.gT('app.Login'), this.gT('app.SessionForUserRestored', { userName: user.userName }),
                MessageSeverity.success);
              setTimeout(() => {
                this.alertService.showStickyMessage(this.gT('app.SessionRestored'), this.gT('app.PleaseRetryOperation'),
  ...
  offerAlternateHost() {
    if (Utilities.checkIsLocalHost(location.origin) && Utilities.checkIsLocalHost(this.configurations.baseUrl)) {
      const apiUrl = prompt('Dear Developer!\nIt appears your backend Web API service is not running...\n' +
        ...
        this.configurations.fallbackBaseUrl ?? undefined);
